...
{
public:
  Getdata() : Node("getdata_node"), last_time_(this->now()), U(VectorXd::Zero(4)), time(0), xr(0), yr(0), zr(0) // Initialize U with size 4
  {
    ...
    trajectory_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
        "/datatraject", 10, std::bind(&Getdata::trajectCallback, this, std::placeholders::_1));

    subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/droneposition/odom", 10, std::bind(&Getdata::topic_callback, this, std::placeholders::_1));

    // Open file in append mode
    outfile_.open("data.txt", std::ios::app);
    if (!outfile_.is_open())
    {
...
private:
  ...
  void trajectCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
  {
    if (msg->data.size() != 4)
    {
      RCLCPP_WARN(this->get_logger(), "Expected 4 trajectory data, but got %zu", msg->data.size());
      return;
    }
    time = msg->data[0];
    xr = msg->data[1];
