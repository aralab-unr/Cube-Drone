...
public:
    NMPCSMCCUBE(): Node("NMPCSMCCUBE"), controlxy_(0.05, 0.1, 0.05, 0.1), xr(5.0), yr(5.0), zr(5.0), psir(0), x(0), y(0), z(0), xd(0), yd(0), zd(0), phi(0), theta(0), psi(0), phid(0), thetad(0), psid(0), w9(0), w10(0), addforce(0), time(0), pretime_(this->get_clock()->now()) {
        ...
        trajectory_pub = this->create_publisher<std_msgs::msg::Float64MultiArray>("/datatraject", 10);
...
private:
    ...
    void stateCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("NMPCSMCCUBE"), "Time now: %f", time);
        if (time <= 10)
        {
            xr = 5.0;
            yr = 5.0;
            ...
            xr = 5 * sin(0.157 * (time - 50));
            yr = 5 * sin(0.314 * (time - 50));
            zr = 5.0;
        }
        else{
            xr = 0.0;
        ...
        trajectory_msg.data = {time, xr, yr, zr};
    ...
    rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr trajectory_pub;
